...
export default function LayoutTemplate({items, newBranches, children}:{items:MenuNavItem[], newBranches:any, children:React.ReactNode}){
    ...
    return <div>
        <header className="sticky top-0 z-50 shadow-lg bg-red-500">
            ...
            <h1 className="self-center text-center mr-4">Shopping Das Carnes</h1>
            <Input placeholder="Pesquisa..." className="w-6/12 self-center drop-shadow-md"/>
            <button className="flex text-xs w-auto mt-8 px-4">
                <span className="flex w-36"><Book className="size-4"/> Histórico de Compras</span>
            </button>
            <button className="flex text-xs w-auto mt-8 pl-2 pr-4">
                <span className="flex w-36"><BaggageClaim className="size-4"/>Carrinho</span>
            </button>
        ...
        {footerInfo.map((j,i)=><FooterInfo key={i} header={'Header'} items={j} className="flex flex-col self-center justify-self-center justify-items-end pr-8 items-start"/>)}
        </footer>
        </div>
